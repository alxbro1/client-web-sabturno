...
export function LocalBlockingsPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const endDate = formData.type === 'full-day' ? new Date(formData.date) : new Date(formData.date);
    ...
    <section className="grid gap-6">
      <header className="flex justify-between items-center">
        <div>
          ...
          <h2 className="text-2xl font-bold text-white">Bloqueos de calendario</h2>
        </div>
      ...
      {blockedDates.length > 0 && (
        <section>
          <h3 className="text-lg font-semibold text-white mb-3">Bloqueos activos</h3>
              ...
              >
                <div>
                  <p className="text-white font-medium">
                    {new Date(block.startDate).toDateString() === new Date(block.endDate).toDateString()
                      ? formatDate(new Date(block.startDate))
                      : `${formatDate(new Date(block.startDate))} - ${formatDate(new Date(block.endDate))}`}
                  </p>
                  {block.reason && <p className="text-sm text-white/50">{block.reason}</p>}
                </div>
                <Button
                  variant="ghost"
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="border border-white/12 bg-[#141414] rounded-[28px] shadow-[0_24px_70px_rgba(0,0,0,0.5)] p-6 w-full max-w-md">
            <h3 className="text-xl font-bold text-white mb-4">Bloquear fecha</h3>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-white/80 mb-2">Tipo de bloqueo</label>
                <div className="flex gap-3">
                  <button
                    type="button"
                    onClick={() => setFormData(prev => ({ ...prev, type: 'full-day' }))}
                  ...
                  <button
                    type="button"
                    onClick={() => setFormData(prev => ({ ...prev, type: 'time-slot' }))}
                    className={`flex-1 p-3 rounded-xl border transition-all ${
              ...
              {formData.type === 'time-slot' && (
                <div className="flex gap-3">
                  <div className="flex-1">
                    <label className="block text-sm font-medium text-white/80 mb-2">Desde</label>
                    <input
                      type="time"
                  ...
                  <div className="flex-1">
                    <label className="block text-sm font-medium text-white/80 mb-2">Hasta</label>
                    <input
                      type="time"
              ...
              <div>
                <label className="block text-sm font-medium text-white/80 mb-2">Motivo (opcional)</label>
                <input
                  type="text"
