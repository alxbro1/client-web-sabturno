...
export function LocalDashboardPage() {
  ...
  const { nextAppointment, todayAppointments, dashboardStats, isLoading, error, refreshData } = useLocalHome();
  ...
  return (
    <section className="grid gap-6">
      <header className="rounded-[28px] border border-white/12 bg-[radial-gradient(circle_at_0%_0%,rgba(0,240,104,0.18),transparent_42%),linear-gradient(180deg,rgba(20,20,20,0.98),rgba(11,11,11,0.96))] shadow-[0_24px_70px_rgba(0,0,0,0.34)] flex justify-between gap-6 items-end p-8 max-sm:flex-col max-sm:items-stretch">
        <div>
          <p className="text-[0.75rem] font-bold uppercase tracking-[0.22em] text-[#00f068] mb-1">Panel del Local</p>
          <h2 className="text-2xl font-bold text-white">{user?.localName || user?.name}</h2>
          <p className="text-white/60 mt-1">Desde aqui puedes gestionar turnos, horarios y mas.</p>
        </div>
        <div className="flex flex-wrap gap-3 items-center">
          <Link to="/local/calendar">
      ...
      {error && (
        <div className="rounded-2xl border border-[#ff5678]/40 bg-[rgba(83,15,34,0.42)] px-4 py-[0.95rem] text-[#ffd6df] flex flex-wrap gap-4 items-center justify-between">
          <span>{error}</span>
          <Button variant="ghost" onClick={() => refreshData()}>Reintentar</Button>
        </div>
      )}
      ...
      <section className="grid grid-cols-4 gap-4 max-lg:grid-cols-2 max-sm:grid-cols-1">
        <LocalStatsCard
          ...
          icon={<IconCalendar />}
        >
          <span className="text-[0.75rem] text-white/40">{new Date().toLocaleDateString('es-AR', { day: 'numeric', month: 'short' })}</span>
        </LocalStatsCard>
      ...
      <section>
        <div className="flex justify-between gap-4 items-start mb-3">
          <div>
            ...
            <h3 className="text-lg font-semibold text-white"> {nextAppointment ? "Siguiente reserva" : "Sin turnos proximos"}</h3>
          </div>
          {isLoading && <span className="text-[0.78rem] uppercase tracking-[0.08em] text-white/52">Actualizando...</span>}
        </div>
        ...
        {nextAppointment ? (
          <article className="border border-[#00f068]/25 bg-[linear-gradient(135deg,rgba(0,240,104,0.08),rgba(0,240,104,0.02))] rounded-[28px] p-6 shadow-[0_8px_32px_rgba(0,240,104,0.15)]">
            <div className="flex justify-between items-start">
              <div>
                <p className="text-[0.8rem] text-white/50 uppercase tracking-wider mb-1">Cliente</p>
                <p className="text-xl font-bold text-white">{nextAppointment.clientName}</p>
                <p className="text-[#00f068] mt-1">{nextAppointment.serviceName}</p>
              </div>
              <div className="text-right">
                <p className="text-[0.8rem] text-white/50 uppercase tracking-wider mb-1">Hora</p>
                <p className="text-lg font-bold text-white">{formatTime(nextAppointment.startDateTime)}</p>
                <p className="text-white/60 text-sm">{formatDateLong(nextAppointment.startDateTime)}</p>
              </div>
            </div>
          </article>
        ) : (
      ...
      <section>
        <div className="flex justify-between gap-4 items-start mb-3">
          <div>
            <p className="text-[0.75rem] font-bold uppercase tracking-[0.22em] text-[#00f068]">Hoy</p>
            <h3 className="text-lg font-semibold text-white">Turnos del dia</h3>
          </div>
          <span className="text-[0.8rem] text-white/50">{todayAppointments.length} turno{todayAppointments.length !== 1 ? 's' : ''}</span>
        </div>

        {todayAppointments.length > 0 ? (
          <div className="grid gap-3">
            {todayAppointments.slice(0, 8).map((appt) => (
              <article key={appt.id} className="flex items-center justify-between p-4 rounded-2xl border border-white/10 bg-white/[0.02] hover:bg-white/[0.04] transition-colors">
                ...
                <div className="flex items-center gap-4">
                  <span className={`px-3 py-1 rounded-full text-xs font-medium border ${statusColors[appt.status] || statusColors.pending}`}>
                    {appt.status === 'pending' ? 'Pendiente' :
                     appt.status === 'confirmed' ? 'Confirmado' :
                     appt.status === 'completed' ? 'Completado' : 'Cancelado'}
                  </span>
                  <div className="text-right">
                    <p className="font-semibold text-white">{formatTime(appt.startDateTime)}</p>
                    <p className="text-sm text-white/50">{formatCurrency(appt.price)}</p>
                  </div>
                </div>
              </article>
            ))}
            {todayAppointments.length > 8 && (
              <Link to="/local/calendar" className="text-center py-3 text-[#00f068] text-sm hover:underline">
