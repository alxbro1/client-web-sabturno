...
const navItems: NavItem[] = [
  ...
  { to: "/local/profile", label: "Perfil", icon: <svg viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5"><path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z" /></svg> },
];

export function LocalOwnerShell() {
  ...
  return (
    <div className="grid grid-cols-[280px_1fr] gap-5 min-h-screen p-5 max-lg:grid-cols-1 max-sm:gap-3 max-sm:p-3">
      <aside className="self-start sticky top-5 h-[calc(100vh-2.5rem)] overflow-y-auto p-6 flex flex-col gap-8 justify-between border border-white/10 bg-[rgba(10,10,10,0.9)] rounded-[28px] shadow-[0_24px_65px_rgba(0,0,0,0.38)] backdrop-blur-md max-sm:hidden">
        <div>
          <Link to="/local/dashboard" className="flex items-center gap-2 mb-4">
            <LogoMark />
          </Link>
          <p className="text-[0.8rem] text-white/48 max-w-[18rem]">Panel de administracion del local</p>
        </div>

        <nav className="grid gap-2">
          {navItems.map((item) => (
            <NavLink key={item.to} className={navLinkClass} to={item.to} onClick={handleMobileNavigation}>
              {item.icon}
        ...
        <div className="grid gap-[0.85rem]">
          <div className="p-3 rounded-[18px] border border-white/10 bg-white/[0.02]">
            <p className="text-[0.75rem] text-white/48 uppercase tracking-wider mb-1">Local</p>
            <strong className="text-[0.95rem] text-white">{user?.localName || user?.name}</strong>
          </div>
          <Button
            variant="ghost"
      ...
      <main className="p-6 border border-white/10 bg-[#0f1014] rounded-[28px] shadow-[0_12px_32px_rgba(0,0,0,0.28)] overflow-y-auto max-sm:order-2 max-sm:p-4 max-sm:rounded-[22px]">
        <div className="hidden max-sm:flex items-center justify-between gap-3 pb-4">
          <Link to="/local/dashboard" className="flex items-center gap-2">
            <LogoMark />
          </Link>
          <button
            type="button"
