...
export function LocalSchedulesPage() {
  const navigate = useNavigate();
  const { templates, isLoading, error, refreshTemplates } = useScheduleTemplateList();
    ...
    <section className="grid gap-6">
      <header className="flex justify-between items-center">
        <div>
          ...
          <h2 className="text-2xl font-bold text-white">Plantillas de Horarios</h2>
        </div>
        <Link to="/local/schedules/edit/new">
          <Button>
      ...
      {isLoading ? (
        ...
        <div className="min-h-[200px] grid place-items-center text-center rounded-[28px] border border-white/10 bg-white/[0.02] p-8">
          ...
          <p className="text-white/60 mb-4">No hay plantillas de horario configuradas.</p>
          <Link to="/local/schedules/edit/new">
          ...
          {templates.map((template) => (
            ...
            >
              <div className="flex items-center gap-4">
                <div className={`w-3 h-3 rounded-full ${template.isActive ? 'bg-[#00f068]' : 'bg-white/30'}`} />
                <div>
                  <h3 className="font-semibold text-white">{template.name}</h3>
                  <p className="text-sm text-white/50">
                    {template.timeSlotsCount} horario{template.timeSlotsCount !== 1 ? 's' : ''} configurado{template.timeSlotsCount !== 1 ? 's' : ''}
                  </p>
                </div>
              </div>
                  ...
                  >
                    <span
                      className={`absolute top-1 w-4 h-4 rounded-full bg-white shadow transition-transform ${
                        template.isActive ? 'translate-x-7' : 'translate-x-1'
                      }`}
                    />
                  </button>
                </label>

                <Link to={`/local/schedules/edit/${template.id}`}>
